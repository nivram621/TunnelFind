...
static CGFloat const kMCStop2                       = 0.45; // Percentage limit to trigger the second action
...
- (void)handlePanGestureRecognizer:(UIPanGestureRecognizer *)gesture {
    ...
    if (state == UIGestureRecognizerStateBegan || state == UIGestureRecognizerStateChanged) {
        ...
        else if (cellState == MCSwipeTableViewCellState4 && _modeForState5) {
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
    ...
    if ([gestureRecognizer class] == [UIPanGestureRecognizer class]) {
        ...
        if (fabs(point.x) > fabs(point.y) ) {
            if (point.x < 0 && !_modeForState3 && !_modeForState5) {
                return NO;
            }
...
- (UIView *)viewWithPercentage:(CGFloat)percentage {
    ...
    if (percentage <= -_secondTrigger && _modeForState5) {
        view = _view5;
    }
